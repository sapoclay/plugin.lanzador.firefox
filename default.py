#!/usr/bin/python
# -*- coding: utf-8 -*-
#default.py es casi una copia del archivo default.py de chrome launcher (lo he modificado un poco solo)
import urllib
import sys
import re
import os
...
def index():
    ...
    addDir("[ Entreunosyceros ]", entreunosycerosUrl, 'showSite', os.path.join(addonPath, "DefaultFolder.png"), "yes", "yes")        
...
def showSite(url, stopPlayback, kiosk, userAgent):
    ...
    if osWin:
        ...
        path = "/Applications/Mozilla Firefox.app/firefox"
        if useCustomPath and os.path.exists(customPath):
            fullUrl = getFullPath(customPath, url, kiosk, userAgent)
